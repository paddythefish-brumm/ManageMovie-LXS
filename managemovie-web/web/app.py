...
def read_public_runtime_settings() -> dict[str, str | bool]:
    return build_public_runtime_settings(
...
def update_runtime_settings(payload: dict | None) -> tuple[bool, str, dict[str, str]]:
    ...
    encoder = normalize_encoder_mode(source.get("encoder", read_last_encoder())) or read_last_encoder()
    ...
    updated["encoder"] = read_last_encoder()
    return True, "", updated
...
def schedule_full_restart() -> tuple[bool, str]:
    ...
    web_port = int(os.environ.get("MANAGEMOVIE_WEB_PORT", "8080") or 8080)
...
def index():
    ...
    response = make_response(
        render_template_string(
            ...
            default_encoder=read_last_encoder(),
...
def start():
    ...
    encoder = normalize_encoder_mode(request.form.get("encoder", "")) or read_last_encoder()
...
def settings_encoder():
    ...
    if not encoder:
        encoder = normalize_encoder_mode(request.form.get("encoder", ""))
    if not encoder:
...
def api_settings_set():
    ...
    return jsonify(
        {
            "ok": True,
            "settings": build_public_runtime_settings(
                updated,
                mode=read_last_mode(),
                encoder=read_last_encoder(),
            ),
...
def api_state():
    ...
    encoder = normalize_encoder_mode(ui_state.get(STATE_KEY_LAST_ENCODER, "")) or "cpu"
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>{{ site_title }} | Restart bestaetigen</title>
  <script>
    (function () {
      try {
        const theme = (localStorage.getItem('managemovie.theme') || 'light').toLowerCase() === 'dark' ? 'dark' : 'light';
  ...
  <style>
    body { font-family: "SF Pro Text", "SF Pro Display", -apple-system, BlinkMacSystemFont, "Helvetica Neue", "Segoe UI", sans-serif; margin: 0; background: linear-gradient(180deg, #edf3ff 0%, #e8eef8 100%); color: #101828; }
    .card { max-width: 620px; margin: 28px auto; background: rgba(255,255,255,0.9); border: 1px solid rgba(70,84,104,0.22); border-radius: 16px; padding: 16px; box-shadow: 0 18px 36px rgba(24,39,75,0.18); backdrop-filter: blur(14px) saturate(130%); }
    .head { display: flex; align-items: center; justify-content: space-between; gap: 8px; margin-bottom: 10px; }
    h1 { margin: 0; font-size: 1.05rem; display: inline-flex; align-items: center; gap: 10px; }
    .run-dot { width: 18px; height: 18px; border-radius: 999px; border: 2px solid rgba(0,0,0,0.2); background: #9aa3ad; box-shadow: 0 0 0 3px rgba(255,255,255,0.9) inset; flex: 0 0 auto; }
    .run-dot.running { background: #29a745; }
...
<body>
  <div class="card">
    ...
    <p>App und DB wirklich neu starten? Alle laufenden Prozesse werden beendet.</p>
    <div class="row">
      <button type="button" onclick="window.close()">Zurueck</button>
...
<body>
  ...
  <div id="inlineConfirmModal" class="log-modal hidden" onclick="if (event.target && event.target.id === 'inlineConfirmModal') resolveInlineConfirm(false);">
    <div class="log-modal-panel confirm-modal-panel">
      ...
      <div class="confirm-modal-actions">
        <button type="button" class="btn btn-primary" onclick="resolveInlineConfirm(false)">Zurück</button>
        <button type="button" class="btn btn-stop" onclick="resolveInlineConfirm(true)">Abbruch</button>
      </div>
    </div>
  </div>
      ...
      <div class="settings-body">
        <div class="settings-grid">
          ...
          <select id="encoderSetting">
            <option value="cpu"{% if default_encoder == "cpu" %} selected{% endif %}>Software</option>
            <option value="intel_qsv"{% if default_encoder == "intel_qsv" %} selected{% endif %}>Intel QuickSync</option>
            <option value="apple"{% if default_encoder == "apple" %} selected{% endif %}>Apple</option>
          </select>
  ...
  <script>
    ...
    function askBrowserConfirm(message, title = 'Bestätigen') {
      const modal = document.getElementById('inlineConfirmModal');
    ...
    function openRestartWindow() {
      const w = window.open('/restart-window', '_blank', 'width=620,height=320');
      if (w) {
        w.focus();
        return;
      }
      window.location.href = '/restart-window';
    }
    ...
    function setEncoderControls(encoder) {
      const value = String(encoder || '').trim();
      if (!value) return;
      const allowed = new Set(['cpu', 'intel_qsv', 'apple']);
...
if __name__ == "__main__":
    ...
    port = int(os.environ.get("MANAGEMOVIE_WEB_PORT", "8080"))
