...
if [ ! -x {q_update} ]; then
  echo "[update] update_ManageMovie.sh fehlt oder ist nicht ausführbar" >> {q_log}
  exit 1
fi
nohup {q_update} >> {q_log} 2>&1 </dev/null &
"""
...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>{{ site_title }} | Update bestaetigen</title>
  <script>
    (function () {
      try {
        const theme = (localStorage.getItem('managemovie.theme') || 'light').toLowerCase() === 'dark' ? 'dark' : 'light';
...
<body>
  <div class="card">
    ...
    <p>Neuestes Release von GitHub holen und App sauber neu starten?</p>
    <div class="row">
      <button type="button" onclick="window.close()">Zurueck</button>
...
<body>
  <div class="grid">
    <div class="card wide">
      ...
      <div class="card-actions">
          ...
          <button type="button" class="log-expand-btn restart-btn" title="Update" aria-label="Update" onclick="openUpdateWindow()">⟳</button>
  ...
  <script>
    ...
    function openUpdateWindow() {
      const w = window.open('/update-window', '_blank', 'width=620,height=320');
      if (w) {
        w.focus();
        return;
      }
      window.location.href = '/update-window';
    }
