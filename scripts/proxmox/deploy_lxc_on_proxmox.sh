...
resolve_common_sh() {
  ...
  for candidate in \
    ...
    "$SCRIPT_DIR/ManageMovie-LXS/scripts/lib/common.sh" \
    "/root/ManageMovie-LXS/scripts/lib/common.sh"
  do
    if [ -f "$candidate" ]; then
      printf '%s\n' "$candidate"
      return 0
...
COMMON_SH="$(resolve_common_sh || true)"
if [ -n "${COMMON_SH:-}" ]; then
  source "$COMMON_SH"
else
  MM_PROJECT_ROOT=""
  for candidate in \
    ...
    "$SCRIPT_DIR/ManageMovie-LXS" \
    "/root/ManageMovie-LXS"
  do
    if [ -f "$candidate/managemovie-web/app/managemovie.py" ]; then
      MM_PROJECT_ROOT="$candidate"
      break
    fi
  done
  if [ -z "$MM_PROJECT_ROOT" ]; then
    echo "Fehler: ManageMovie-Projektwurzel wurde nicht gefunden. Erwartet wird das Repo z.B. unter /root/ManageMovie-LXS." >&2
    exit 1
  fi
...
SAFE_PCT_SCRIPT="$PROJECT_ROOT/scripts/proxmox/safe_pct_lxc.sh"
...
CONTAINER_USE_DHCP="${MANAGEMOVIE_LXC_DHCP:-0}"
...
Options:
  ...
  --dhcp              IPv4 und Gateway per DHCP beziehen
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --dhcp) CONTAINER_USE_DHCP=1; shift ;;
...
safe_pct_run() {
  local action="$1"
  local ctid="$2"
  if [ -x "$SAFE_PCT_SCRIPT" ]; then
    "$SAFE_PCT_SCRIPT" "$action" "$ctid"
    return
  fi
  case "$action" in
    start) pct start "$ctid" ;;
    stop) pct stop "$ctid" ;;
    destroy) pct destroy "$ctid" --purge 1 ;;
    cleanup) true ;;
    restart)
      ...
      ;;
    *)
      echo "Fehler: unbekannte safe_pct_run Aktion: $action" >&2
      return 1
      ;;
  esac
}
...
container_net0_config() {
  if [ "$CONTAINER_USE_DHCP" = "1" ]; then
    printf '%s\n' "name=eth0,bridge=vmbr0,ip=dhcp,type=veth"
  else
    printf '%s\n' "name=eth0,bridge=vmbr0,gw=${CONTAINER_GATEWAY},ip=${CONTAINER_IP}/24,type=veth"
  fi
}

current_container_ip() {
  local ctid="$1"
  pct exec "$ctid" -- bash -lc "ip -4 -o addr show dev eth0 scope global | awk '{print \$4}' | cut -d/ -f1 | head -n 1" 2>/dev/null | tr -d '\r'
}
...
if pct status "$CONTAINER_CTID" >/dev/null 2>&1; then
  ...
  if [ "$EXISTING_LOCK_STATE" = "create" ] && [ "$EXISTING_STATUS_STATE" = "stopped" ]; then
    ...
    safe_pct_run destroy "$CONTAINER_CTID" >/dev/null 2>&1 || true
    CONTAINER_ACTION="create"
    ...
    safe_pct_run cleanup "$CONTAINER_CTID" >/dev/null 2>&1 || true
    pct unlock "$CONTAINER_CTID" >/dev/null 2>&1 || true
...
if pct status "$CONTAINER_CTID" >/dev/null 2>&1; then
  if [ "$ACTION_MODE" = "recreate" ] || [ "$RECREATE" -eq 1 ]; then
    ...
    safe_pct_run stop "$CONTAINER_CTID" >/dev/null 2>&1 || true
    safe_pct_run destroy "$CONTAINER_CTID"
  else
...
safe_pct_run start "$CONTAINER_CTID" >/dev/null 2>&1 || true
...
internal_ready=0
for _ in $(seq 1 40); do
  if pct exec "$CONTAINER_CTID" -- bash -lc "curl -kfsS --max-time 5 https://127.0.0.1:${WEB_PORT}/api/state >/dev/null" >/dev/null 2>&1; then
    internal_ready=1
    break
  fi
  sleep 3
...
if [ "$internal_ready" -ne 1 ]; then
  echo "Fehler: Container-Webservice antwortet intern nicht unter https://127.0.0.1:${WEB_PORT}/api/state" >&2
  exit 1
fi

external_ready=0
RUNTIME_IP="$CONTAINER_IP"
if [ "$CONTAINER_USE_DHCP" = "1" ]; then
  RUNTIME_IP=""
  for _ in $(seq 1 40); do
    RUNTIME_IP="$(current_container_ip "$CONTAINER_CTID" || true)"
    if is_ipv4_address "$RUNTIME_IP"; then
      break
    fi
    sleep 3
  done
...
for _ in $(seq 1 40); do
  if [ -n "$RUNTIME_IP" ] && curl -kfsS --max-time 5 "https://${RUNTIME_IP}:${WEB_PORT}/api/state" >/dev/null 2>&1; then
    external_ready=1
    break
  fi
  sleep 3
...
if [ "$external_ready" -ne 1 ]; then
  echo "[warn] Container-Webservice intern gesund, aber vom Proxmox-Host nicht unter https://${RUNTIME_IP:-unbekannt}:${WEB_PORT}/api/state erreichbar." >&2
fi

echo "LXC bereit:"
echo "  Name:    ${CONTAINER_NAME}"
echo "  CTID:    ${CONTAINER_CTID}"
echo "  IP:      ${RUNTIME_IP:-$CONTAINER_IP}"
echo "  Gateway: ${CONTAINER_GATEWAY}"
echo "  Storage: ${CONTAINER_STORAGE}"
echo "  Template-Storage: ${TEMPLATE_STORAGE}"
echo "  Seed:    ${SEED_MODE}"
echo "  Aktion:  ${CONTAINER_ACTION}"
echo "  URL:     https://${RUNTIME_IP:-$CONTAINER_IP}:${WEB_PORT}/"
