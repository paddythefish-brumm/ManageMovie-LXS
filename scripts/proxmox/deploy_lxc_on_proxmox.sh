...
create_template_alias_for_legacy_proxmox() {
  local template_name="$1"
  local current_path=""
  local alias_name=""
  local alias_path=""

  current_path="$(template_cache_path "$template_name")"
  if [ -z "$current_path" ] || [ ! -f "$current_path" ]; then
    return 1
  fi

  alias_name="$(printf '%s\n' "$template_name" | sed -E 's/_(13)\.[0-9]+-/_\1.0-/')"
  if [ -z "$alias_name" ] || [ "$alias_name" = "$template_name" ]; then
    return 1
  fi

  alias_path="$(dirname "$current_path")/$alias_name"
  if [ ! -e "$alias_path" ]; then
    cp -f "$current_path" "$alias_path"
  fi
  printf '%s\n' "$alias_name"
}
...
create_container_with_template_fallback() {
  ...
  if grep -q "unsupported debian version '13\\." "$create_log"; then
    local alias_name=""
    alias_name="$(create_template_alias_for_legacy_proxmox "$template_name" || true)"
    if [ -n "$alias_name" ]; then
      echo "[template] Proxmox akzeptiert ${template_name} nicht direkt. Nutze Debian-13-Alias ${alias_name} fuer Legacy-Hosts."
      if create_container_with_template "$alias_name" >"$create_log" 2>&1; then
        cat "$create_log"
        return 0
      fi
    fi
...
TEMPLATE_STORAGE="$(find_template_storage || true)"
if [ -z "$TEMPLATE_STORAGE" ]; then
  echo "Fehler: Kein Proxmox-Storage mit Content-Typ 'vztmpl' gefunden. Bitte z. B. 'local' fuer Container-Templates aktivieren oder --template-storage setzen." >&2
  exit 1
fi

TEMPLATE="$(pveam update >/dev/null 2>&1 && pveam available | awk '/debian-13-standard_/ {print $2}' | tail -n 1)"
if [ -z "$TEMPLATE" ]; then
...
if ! pveam download "$TEMPLATE_STORAGE" "$TEMPLATE" >/dev/null 2>&1; then
  echo "Fehler: Debian-13-Template konnte nicht nach '${TEMPLATE_STORAGE}' geladen werden." >&2
  exit 1
fi
