...
upgrade_to_debian_13() {
  ...
  if [ -f /etc/apt/sources.list ]; then
    sed -i 's/bookworm/trixie/g; s/bullseye/trixie/g' /etc/apt/sources.list
  fi
  if [ -d /etc/apt/sources.list.d ]; then
    find /etc/apt/sources.list.d -type f \( -name '*.list' -o -name '*.sources' \) -print0 | while IFS= read -r -d '' file; do
      sed -i 's/bookworm/trixie/g; s/bullseye/trixie/g' "$file"
    done
  fi
  "${APT_GET[@]}" update
...
systemctl --no-pager --full status managemovie-web.service | sed -n '1,18p'
